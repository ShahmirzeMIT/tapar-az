...
export default function Listings() {
  ...
  return <main className="min-h-screen bg-offwhite dark:bg-background"><div className="mx-auto max-w-7xl px-6 py-8 md:py-12">
    <div className="mb-8 rounded-2xl bg-[#611F69] p-5 text-white shadow-card dark:bg-gradient-to-br dark:from-[#070709] dark:via-[#141418] dark:to-[#202027] dark:text-white md:p-7"><p className="text-xs font-bold uppercase tracking-[.18em] text-[#f7edf9] dark:text-[#FB923C]">TAPAR.AZ Axtarış</p><h1 className="mt-2 font-display text-3xl font-bold md:text-4xl">{activeSearch ? `“${activeSearch}” üçün nəticələr` : 'Bütün elanlar'}</h1><div className="mt-5 flex max-w-3xl rounded-xl border border-white/70 bg-white/95 p-1 shadow-lg shadow-[#3c1241]/10 dark:border-white/10 dark:bg-[#1B1B20] dark:shadow-black/25"><Input bordered={false} prefix={<SearchOutlined className="text-[#8d5b94] dark:text-[#FB923C]" />} value={term} onChange={(e) => setTerm(e.target.value)} onPressEnter={search} placeholder="Elan, marka və ya model axtarın..." className="flex-1 !bg-transparent !text-[#3c1241] dark:!text-white" /><button onClick={search} className="inline-flex items-center justify-center rounded-lg bg-[#3c1241] px-5 py-2 text-sm font-semibold text-white transition hover:bg-[#2b0d2f] dark:bg-[#F97316] dark:text-[#070709] dark:hover:bg-[#FB923C]">Axtar</button></div></div>
