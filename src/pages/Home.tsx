...
import {
  ...
  CheckCircleFilled, EnvironmentOutlined, HeartFilled, TagOutlined, UsergroupAddOutlined,
  DatabaseOutlined,
...
function CategoryIcon({ name }: { name: string }) {
  const icons: Record<string, ReactNode> = {
    home: <HomeOutlined />, real_estate: <HomeOutlined />, transport: <CarOutlined />,
    electronics: <LaptopOutlined />, services: <ToolOutlined />, hobby: <GiftOutlined />,
  };
  return icons[name] ?? <GiftOutlined />;
}

function SectionHeading({ title, linkTo }: { title: string; linkTo: string }) {
  return <div className="mb-5 flex items-center justify-between"><h2 className="font-display text-2xl font-bold tracking-tight text-ink dark:text-white">{title}</h2><Link to={linkTo} className="text-sm font-semibold text-muted hover:text-action">Hamısına bax <ArrowRightOutlined /></Link></div>;
}

function ListingGrid({ listings, loading }: { listings: Parameters<typeof ListingCard>[0]['listing'][]; loading: boolean }) {
  if (loading) return <div className="grid grid-cols-2 gap-4 md:grid-cols-4">{[1, 2, 3, 4].map((item) => <Skeleton active key={item} />)}</div>;
  return <div className="grid grid-cols-2 gap-4 md:grid-cols-4">{listings.slice(0, 4).map((item) => <ListingCard key={item.id} listing={item} />)}</div>;
}

function TrustItem({ icon, title, text }: { icon: ReactNode; title: string; text: string }) {
  return <div className="flex gap-4"><span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-action/10 text-xl text-action">{icon}</span><div><h3 className="font-bold text-ink dark:text-white">{title}</h3><p className="mt-1 text-sm leading-6 text-muted">{text}</p></div></div>;
}
...
export default function Home() {
  ...
  const { listings, loading } = useExternalListings();
  const t = (key: string) => ({
    heroKicker: 'Al, sat, rahat tap', heroTitle: 'Azərbaycanda al və sat', heroText: 'İstədiyiniz məhsulu tapın, elanınızı saniyələr içində yerləşdirin.', searchPlaceholder: 'Nə axtarırsınız?', search: 'Axtar', popularCategories: 'Populyar kateqoriyalar', viewAll: 'Hamısına bax', browseAds: 'Elanlara bax', latest: 'Ən son elanlar', why: 'Niyə TAPAR.AZ?', trustTitle: 'Axtardığını tap, güvənlə al', trust1: 'Daha güvənli alış-veriş', trust2: 'Axtardığınızı tez tapın', trust3: 'Aktiv alıcı icması', trustText1: 'Telefonu təsdiqlənmiş satıcıları asanlıqla tanıyın.', trustText2: 'Ağıllı filterlər və kateqoriyalarla vaxtınıza qənaət edin.', trustText3: 'Elanınızı minlərlə potensial alıcıya göstərin.', sellEasy: 'Elanınızı asanlıqla yerləşdirin', placeYourAd: 'Elan yerləşdir', ...(language === 'en' ? { heroTitle: 'Buy and sell in Azerbaijan', search: 'Search' } : {}), ...(language === 'ru' ? { heroTitle: 'Покупайте и продавайте в Азербайджане', search: 'Поиск' } : {}),
        ...
        <div className="mx-auto grid max-w-7xl items-center gap-12 px-6 py-14 md:grid-cols-[1.02fr_.98fr] md:gap-16 md:py-24">
          <div className="max-w-2xl">
            ...
            <div className="mt-8 flex max-w-xl rounded-xl border border-line bg-paper p-1.5 shadow-[0_14px_35px_rgb(17_24_39/0.09)] dark:border-line-dark dark:bg-graphite">
              ...
              />
              <button onClick={goSearch} className="market-action shrink-0 px-5 md:px-7">
