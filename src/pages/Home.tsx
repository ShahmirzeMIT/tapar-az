    ...
    <div>
      ...
      <section className="overflow-hidden bg-[#FF6C2C]">
        <div className="mx-auto grid max-w-7xl items-center gap-10 px-6 py-12 md:grid-cols-[minmax(0,1fr)_minmax(360px,500px)] md:py-16 lg:gap-16">
          ...
          <div className="relative pb-12 sm:pb-14"><div className="absolute -inset-5 rounded-[2rem] bg-[#16A34A]/10 blur-2xl" /><div className="relative overflow-hidden rounded-[1.75rem] border border-white/80 bg-white p-3 shadow-[0_25px_70px_rgba(22,163,74,.18)] dark:border-line-dark dark:bg-background"><div className="mb-3 flex items-center justify-between px-2"><div><p className="text-[10px] font-bold uppercase tracking-[.18em] text-[#16A34A]">Təzə elanlar</p><p className="mt-1 font-display text-lg font-bold text-ink dark:text-white">Son əlavə edilənlər</p></div><Link to="/elanlar" className="text-xs font-semibold text-[#16A34A]">Hamısına bax <ArrowRightOutlined /></Link></div><div>{latestLoading ? <div className="aspect-[4/3] animate-pulse rounded-2xl bg-offwhite dark:bg-graphite" /> : latestTen.length ? <Carousel autoplay autoplaySpeed={4200} pauseOnHover dots={{ className: '!bottom-3' }} className="hero-listing-carousel">{latestTen.map((listing) => <HeroListing key={listing.id} listing={listing} />)}</Carousel> : <div className="flex aspect-[4/3] items-center justify-center rounded-2xl bg-offwhite text-sm text-muted dark:bg-graphite">Hələ aktiv elan yoxdur</div>}</div></div><div className="absolute bottom-0 left-3 right-3 z-10 grid grid-cols-3 gap-2 sm:left-5 sm:right-5"><HeroStat value={latest.length ? `${latest.length}+` : '0'} label="Aktiv elan" /><HeroStat value={userCount == null ? '—' : `${userCount}+`} label="İstifadəçi" /><HeroStat value="24/7" label="Axtarış imkanı" /></div></div>
        </div>
      </section>
...
function HeroStat({ value, label }: { value: string; label: string }) {
  return <div className="rounded-xl border border-[#16A34A]/20 bg-[#16A34A]/5 px-2 py-2.5 shadow-[0_8px_20px_rgba(22,163,74,.08)]"><p className="font-display text-base font-bold text-[#16A34A] sm:text-xl">{value}</p><p className="mt-0.5 truncate text-[9px] font-medium text-ink/70 sm:text-[10px]">{label}</p></div>;
}
