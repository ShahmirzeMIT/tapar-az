...
export default function Home() {
  ...
  return (
    <div>
      {/* HERO */}
      <section className="overflow-hidden bg-[#FF6C2C]">
        <div className="mx-auto grid max-w-7xl items-center gap-10 px-6 py-12 md:grid-cols-[minmax(0,1fr)_minmax(360px,500px)] md:py-16 lg:gap-16">
          ...
          <div className="relative"><div className="absolute -inset-6 rounded-[2.5rem] bg-[#16A34A]/15 blur-3xl" /><div className="relative overflow-hidden rounded-[2rem] border border-white/90 bg-white p-4 shadow-[0_25px_70px_rgba(22,163,74,.22)] dark:border-line-dark dark:bg-background"><div className="mb-4 flex items-center justify-between px-1"><div><div className="inline-flex items-center gap-2 rounded-full bg-[#16A34A]/10 px-3 py-1 text-[10px] font-bold uppercase tracking-[.16em] text-[#16A34A]"><span className="h-1.5 w-1.5 rounded-full bg-[#16A34A]" />Canlı elanlar</div><p className="mt-2 font-display text-xl font-bold text-ink dark:text-white">Son əlavə edilənlər</p></div><Link to="/elanlar" className="inline-flex items-center gap-1 rounded-full border border-[#16A34A]/20 px-3 py-1.5 text-xs font-bold text-[#16A34A] transition hover:bg-[#16A34A]/10">Hamısına bax <ArrowRightOutlined /></Link></div><div>{latestLoading ? <div className="aspect-[4/3] animate-pulse rounded-[1.25rem] bg-offwhite dark:bg-graphite" /> : latestTen.length ? <Carousel autoplay autoplaySpeed={4200} pauseOnHover dots={{ className: '!bottom-3' }} className="hero-listing-carousel">{latestTen.map((listing) => <HeroListing key={listing.id} listing={listing} />)}</Carousel> : <div className="flex aspect-[4/3] items-center justify-center rounded-[1.25rem] bg-offwhite text-sm text-muted dark:bg-graphite">Hələ aktiv elan yoxdur</div>}</div></div><div className="mt-5 flex items-center justify-center gap-3 sm:gap-5"><HeroStat value={latest.length ? `${latest.length}+` : '0'} label="Aktiv elan" delay="0ms" /><HeroStat value={userCount == null ? '—' : `${userCount}+`} label="İstifadəçi" delay="180ms" /><HeroStat value="24/7" label="Axtarış imkanı" delay="360ms" /></div></div>
        </div>
      </section>
...
function HeroStat({ value, label, delay }: { value: string; label: string; delay: string }) {
  return <div style={{ animationDelay: delay }} className="hero-stat-float flex h-[88px] w-[88px] shrink-0 flex-col items-center justify-center rounded-full border-2 border-[#16A34A]/25 bg-white text-center shadow-[0_12px_28px_rgba(22,163,74,.2)] sm:h-[104px] sm:w-[104px]"><p className="font-display text-xl font-extrabold leading-none text-[#16A34A] sm:text-2xl">{value}</p><p className="mt-2 max-w-[80px] text-[9px] font-semibold leading-tight text-ink/70 sm:text-[10px]">{label}</p></div>;
}
...
function HeroListing({ listing }: { listing: import('@/types').Listing }) {
  ...
  return <Link to={`/elanlar/${listing.id}`} className="group block overflow-hidden rounded-[1.35rem] border border-line bg-white shadow-[0_12px_32px_rgba(17,24,39,.1)] transition duration-300 hover:-translate-y-1 hover:shadow-[0_18px_42px_rgba(17,24,39,.15)] dark:border-line-dark dark:bg-graphite"><div className="relative aspect-[1.45] overflow-hidden bg-[#eee8e2] dark:bg-background">{image ? <img src={image} alt={listing.title} className="h-full w-full object-cover transition duration-700 ease-out group-hover:scale-105" /> : <div className="flex h-full items-center justify-center text-sm text-muted">Şəkil yoxdur</div>}</div><div className="p-4"><h3 className="line-clamp-1 font-display text-base font-bold text-ink transition group-hover:text-[#16A34A] dark:text-white">{listing.title}</h3><div className="mt-3 flex items-center justify-between gap-3"><p className="font-display text-xl font-extrabold text-[#16A34A]">{listing.price == null ? 'Razılaşma' : formatPrice(listing.price)}</p><span className="flex h-9 w-9 items-center justify-center rounded-full bg-[#16A34A]/10 text-[#16A34A] transition group-hover:bg-[#16A34A] group-hover:text-white"><ArrowRightOutlined /></span></div><div className="mt-3 flex items-center gap-1.5 border-t border-line pt-3 text-xs text-muted dark:border-line-dark"><EnvironmentOutlined /> {listing.city}</div></div></Link>;
}
